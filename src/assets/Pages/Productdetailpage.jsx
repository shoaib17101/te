...
const product=Products.filter(pr=>pr.handleId===handleId).map((pr)=>(
    <section className='flex justify-center gap-4 p-10 '>
        <div className='w-1/3'>
     <div key={pr.handleId} className="p-3 gap ">
        ...
        <div className="w-1/2 flex flex-col gap-3">
            ...
            <h2>Quantity</h2>
            <span className="join flex bg-white items-center justify-between w-1/6"><button onClick={()=>quantity!=0 && setquantity(q=>q-1)} className="btn btn-ghost">-</button > <p className='join-item'>{quantity}</p><button onClick={()=>setquantity(q=>q+1)} className="btn btn-ghost">+</button></span>
            <button className="btn btn-primary px-10 p-5 w-1/3 rounded-full" >Add to Cart</button>
            <button className="btn btn-primary px-10 p-5 w-1/3 rounded-full text-white bg-black">Buy Now</button>
            <h2>Product Info</h2>
            <details className='w-1/3'>{pr.additionalInfoDescription1}</details>
            <h2>Refund  Policy</h2>
            <details className='w-1/3'>{pr.additionalInfoDescription2}</details>
            <h2>Shipping Info</h2>
            <details className='w-1/3'>{pr.additionalInfoDescription3}</details>
